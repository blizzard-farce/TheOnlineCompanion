    ...
    <div>
        <div id="login">
            ...
            <asp:Panel ID="registerPanel" runat="server" Visible="False" Width="256px">
                <asp:DetailsView ID="registerView" runat="server" DataSourceID="SqlDataSource1" Height="50px" Width="125px" AutoGenerateRows="False" DefaultMode="Insert">
                    <Fields>
                ...
                <asp:SqlDataSource ID="AccountCreateSource" runat="server" ConnectionString="<%$ ConnectionStrings:DNDConnectionString %>" InsertCommand="INSERT INTO USER (USER_ID, USERNAME, EMAIL, PASSWORD, NAME_FIRST, NAME_LAST) VALUES (?, ?, ?, ?, ?, ?)" ProviderName="<%$ ConnectionStrings:DNDConnectionString.ProviderName %>" SelectCommand="SELECT * FROM USER" DeleteCommand="DELETE FROM USER WHERE USER_ID = ?" UpdateCommand="UPDATE USER SET USERNAME = ?, EMAIL = ?, PASSWORD = ?, NAME_FIRST = ?, NAME_LAST = ? WHERE USER_ID = ?" >
                    <DeleteParameters>
